...
class UnpaidActivity : AppCompatActivity() {
    ...
    private lateinit var tvLoansCount: TextView
    ...
    private fun initializeViews() {
        ...
        tvLoansCount = findViewById(R.id.tvLoansCount)
    ...
    private fun setupRecyclerView() {
        ...
        rvAllHistory.adapter = transactionAdapter
    }
    ...
    private fun setupFirebaseListener() {
        ...
        // This query correctly fetches only UNPAID loans (paid = false)
        val query = historyRef.orderByChild("paid").equalTo(false)

        query.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                transactionList.clear()
                var totalUnpaidAmount = 0.0
                var unpaidLoansCount = 0
                ...
                for (transactionSnapshot in snapshot.children) {
                    ...
                    if (loanId != null && transaction != null) {
                        ...
                        totalUnpaidAmount += transaction.amount
                        unpaidLoansCount++
                    }
                }
                ...
                // Update summary cards with unpaid loans data
                updateSummaryCards(unpaidLoansCount, totalUnpaidAmount)
    ...
    private fun updateEmptyState() {
        if (filteredTransactionList.isEmpty()) {
            if (etSearch.text.isNotEmpty()) {
                ...
                findViewById<TextView>(R.id.tvEmptySubtitle).text = "No loans match your search criteria."
            } else {
                // Show default empty state
                findViewById<TextView>(R.id.tvEmptyTitle).text = "No Unpaid Loans"
                findViewById<TextView>(R.id.tvEmptySubtitle).text = "All loans are currently paid up to date."
            }
    ...
    private fun updateSummaryCards(loanCount: Int, totalAmount: Double) {
        tvLoansCount.text = loanCount.toString()
        tvTotalUnpaid.text = String.format("₱%.2f", totalAmount)
    }
